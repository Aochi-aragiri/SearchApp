...
function updateProgress() {
  ...
  const completed = items.filter((item) => item.isCompleted).length; // фільтрація айтемів з увімкненими чекбоксами
...
function createItemElem(item) {
  ...
  deleteBtnElem.className = "button is-success is-dark js-delete-btn";
...
function renderItems(filteredItems) {
  ...
  filteredItems
    .map((item) => createItemElem(item)) // створюємо елементи
    .forEach((elem) => refs.list.append(elem)); // додаємо їх у DOM
